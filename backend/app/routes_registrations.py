...
from .models import User, Registration, Attachment, Notification, get_session
from .utils import _save_attachment_file, push_notification_db

router = APIRouter()


@router.post("/login")
def login(data: dict, response: Response, session=Depends(get_session)):
...
def create_registration(data: dict, response: Response, x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
...
def list_registrations(x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    user = session.get(User, x_user_email)
    if not user:
...
def get_notifications(x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    if not x_user_email:
...
@router.get("/registrations/{reg_id}/attachments")
def list_registration_attachments(reg_id: int, x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    user = session.get(User, x_user_email)
    if not user:
        return {"attachments": []}
...
def download_attachment(att_id: int, x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    user = session.get(User, x_user_email)
    if not user:
...
def approve_registration(reg_id: int, data: dict = Body(None), response: Response = None, x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    user = session.get(User, x_user_email)
...
def reject_registration(reg_id: int, data: dict = Body(None), response: Response = None, x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    user = session.get(User, x_user_email)
    if not user or user.role != "Supervisor":
        response.status_code = status.HTTP_403_FORBIDDEN
...
@router.patch("/registrations/{reg_id}/verify")
def verify_registration(reg_id: int, response: Response, x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    user = session.get(User, x_user_email)
...
@router.patch("/registrations/{reg_id}/unverify")
def unverify_registration(reg_id: int, response: Response, x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    user = session.get(User, x_user_email)
    if not user or user.role != "Coordinator":
        response.status_code = status.HTTP_403_FORBIDDEN
...
def delete_registration(reg_id: int, response: Response, x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    user = session.get(User, x_user_email)
    if not user or user.role != "Student":
        response.status_code = status.HTTP_403_FORBIDDEN
...
def update_registration(reg_id: int, data: dict, response: Response, x_user_email: str = Header(None, alias="X-User-Email"), session=Depends(get_session)):
    user = session.get(User, x_user_email)
    if not user or user.role != "Student":
        response.status_code = status.HTTP_403_FORBIDDEN
